...
const reducer = createReducer<SwapState>(initialState, (builder) =>
  builder
    ...
    .addCase(updatePairData, (state, { payload: { pairData, pairId, timeWindow } }) => {
      ...
      if (!state.pairDataById?.[pairId as any]) {
        state.pairDataById[pairId as any] = {}
      }
      state.pairDataById[pairId as any][timeWindow] = pairData
    })
    .addCase(updateDerivedPairData, (state, { payload: { pairData, pairId, timeWindow } }) => {
      ...
      if (!state.derivedPairDataById[pairId as any]) {
        state.derivedPairDataById[pairId as any] = {}
      }
      state.derivedPairDataById[pairId as any][timeWindow] = pairData
    }),
)
