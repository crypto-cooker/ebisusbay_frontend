...
const Rugsurance = () => {
  ...
  const calculateBurnEligibility = async () => {
    ...
    try {
      ...
      const nftsFromWallet = await getCRC721NftsFromWallet(sscAddress, user.address);
      const nftsFromCsv = await getCRC721NftsFromIds(sscAddress, eligibleIds);
  ...
  const selectNft = (nft) => {
    ...
    console.log('ASD', currentSelectedNfts)
  ...
  const executeBurn = async () => {
    ...
    try {
      const tx = await writeContract.claimRefund(
        user.address,
        selectedNfts.map((n) => n.id)
      );
      await audio.play();
      await fadeAudio();
      setIsBurning(true);
  ...
  const setApprovalForAll = async () => {
    ...
    const isApproved = await sscContract.isApprovedForAll(user.address, rugContractAddress);
    if (!isApproved) {
      let tx = await sscContract.setApprovalForAll(rugContractAddress, true, txExtras);
      await tx.wait();
    }
  };
  ...
  return (
    <div>
      ...
      <section className="gl-legacy container">
        <div className="row">
          <div className="col-lg-12">
            <p className="text-center">
              SSC NFTs can only be refunded to the wallet they were originally minted from. Any unselectable NFTs
                  ...
                  <>
                    ...
                    {!isApproved && (
                      <>
                        ...
                        <ActionButton title="Approve" workingTitle="Approving" extraClass="mx-auto" onClick={approve} />
                      </>
                    )}
                  </>
                ) : (
        ...
        {initialSearchComplete && !isSearching && (
          <>
            <div className="row">
              <div className="col">
                <Heading as="h3"  size="md">Tokens Refundable</Heading>
                ...
                <div className="card-group">
                  {nfts.map((nft, index) => (
                    <div key={index} className="d-item col-xl-3 col-lg-4 col-md-6 col-sm-6 col-xs-12 mb-4 px-2">
                      <div className="card eb-nft__card h-100 shadow" style={selectedNfts.map((o) => o.id).includes(nft.id) ? {border:`2px solid ${getTheme(user.theme).colors.borderColor2}`} : {}}>
                        {nft.isEligible ? (
                          <img src={nft.image} className={`card-img-top`} alt={nft.name} />
                        ) : (
                          <GreyscaleImg src={nft.image} className={`card-img-top`} alt={nft.name} />
                        )}
                          ...
                          {nft.isEligible ? (
                            <div className="nft__item_action">
                              <span style={{ cursor: 'pointer' }} onClick={() => selectNft(nft)}>
                                {selectedNfts.map((o) => o.id).includes(nft.id) ? <>Unselect</> : <>Select for Burn</>}
                              </span>
                            </div>
                          ) : (
                            <span className="text-grey" style={{ fontSize: '14px' }}>
                              Cannot be selected for Burn
                            </span>
                          )}
                        </div>
                      </div>
            ...
            {nfts.length > 0 && (
              <div className="row">
                <div className="col d-flex flex-row justify-content-end">
                  ...
                  <button
                    className="btn-main lead mr15 ms-4 my-auto"
                    onClick={() => setOpenConfirmationDialog(true)}
                    disabled={selectedNfts < 1}
                  >
                    Process Refund
                  </button>
                </div>
        ...
        {initialSearchComplete && !isSearching && (
          <>
            <div className="row">
              <div className="col">
                <Heading as="h3"  size="md">Tokens NOT Refundable</Heading>
      ...
      <Modal onClose={() => setOpenConfirmationDialog(false)} isOpen={openConfirmationDialog} size="2xl" isCentered>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader className="text-center">
            Are you sure you want to burn {selectedNfts.length} SSC {selectedNfts.length === 1 ? 'NFT' : 'NFTs'}?
          </ModalHeader>
...
const ActionButton = ({
  ...
  return (
    <Button type={type} className={extraClass} style={extraStyle} onClick={doWork} disabled={isWorking}>
      {isWorking ? (
        <>
          {workingTitle}...
          <Spinner size='sm' />
