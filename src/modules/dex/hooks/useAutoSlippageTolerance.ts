...
import {ClassicTrade} from "@dex/state/routing/types";
import {L2_CHAIN_IDS, SUPPORTED_GAS_ESTIMATE_CHAIN_IDS} from "@dex/constants/chains";
import {useNetwork} from "wagmi";
import {useStablecoinAmountFromFiatValue} from "@dex/hooks/useStablecoinPrice";
import useGasPrice from "@dex/hooks/useGasPrice";
...
export default function useClassicAutoSlippageTolerance(trade?: ClassicTrade): Percent {
  ...
  const supportsGasEstimate = useMemo(() => chain?.id && SUPPORTED_GAS_ESTIMATE_CHAIN_IDS.includes(chain.id), [chain?.id])
  ...
  return useMemo(() => {
    ...
    const dollarCostToUse =
      chain?.id && SUPPORTED_GAS_ESTIMATE_CHAIN_IDS.includes(chain.id) && gasEstimateUSD
        ? gasEstimateUSD
  ...
  }, [trade, onL2, chain?.id, gasEstimateUSD, gasCostStablecoinAmount, outputDollarValue])
}
