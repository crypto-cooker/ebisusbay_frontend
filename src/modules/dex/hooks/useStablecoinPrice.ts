...
import { ClassicTrade, INTERNAL_ROUTER_PREFERENCE_PRICE } from '@dex/state/routing/types'
import { useRoutingAPITrade } from '@dex/state/routing/useRoutingAPITrade'

import { SupportedInterfaceChain, asSupportedChain } from '@dex/constants/chains'
import {
  CUSD_CELO,
  CUSD_CELO_ALFAJORES,
  DAI_OPTIMISM,
...
export function useStablecoinAmountFromFiatValue(fiatValue: number | null | undefined) {
  ...
  return useMemo(() => {
    if (fiatValue === null || fiatValue === undefined || !chain?.id || !stablecoin) {
      return undefined
    }
